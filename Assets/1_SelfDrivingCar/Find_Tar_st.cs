...
public class Find_Tar_st: MonoBehaviour
{
    ...
    void Update()
    {
        ...
        for (int i = 0; i < linesArray.Length; i = i + 4)
        {
            ...
            {
                //각제한
                if (130 < Mathf.Atan2((linesArray[i + 2] - linesArray[i + 0]), (linesArray[i + 3] - linesArray[i + 1])) * Mathf.Rad2Deg)
                {
                    if (170 > Mathf.Atan2((linesArray[i + 2] - linesArray[i + 0]), (linesArray[i + 3] - linesArray[i + 1])) * Mathf.Rad2Deg)
                    {
                        Imgproc.line(imgMat, new Point(linesArray[i + 0], linesArray[i + 1]), new Point(linesArray[i + 2], linesArray[i + 3]), new Scalar(255, 0, 0), 4);
                        line_count++;
                        is_Right_Line = true;
